...
from employee_department.emp_dept_service import assign_department_service, get_employee_departments_service, remove_department_service
...
@router.post("/{employee_id}/{department_id}",status_code=status.HTTP_201_CREATED,dependencies=[])
async def assign_department(employee_id:int,department_id:int,db:AsyncSession=Depends(get_db)):
...
async def remove_department(employee_id:int,department_id:int,db:AsyncSession=Depends(get_db)):
