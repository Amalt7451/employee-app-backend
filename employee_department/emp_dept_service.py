...
from employee_department.emp_dept_repo import (
    ...
    get_departments_of_employee_repo,
    get_employees_of_department_repo,
...
async def get_departments_of_employee_service(employee_id: int, db):
    if employee_id <= 0:
        raise BadRequestException("invalid employee id")
    return await get_departments_of_employee_repo(employee_id, db)
...
async def get_employees_of_department_service(department_id: int, db):
    if department_id <= 0:
        raise BadRequestException("invalid department id")
    return await get_employees_of_department_repo(department_id, db)
