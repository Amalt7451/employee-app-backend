...
async def get_all_address_repo(id: int, db: AsyncSession):
    stmt = select(Address).where(
        Address.employee_id == id, Address.deleted_at.is_(None)
    )
...
async def update_address_repo(
    address_id: int, country: str, city: str, zipcode: str, db: AsyncSession
):
    stmt = select(Address).where(Address.id == address_id, Address.deleted_at.is_(None))
    address = await db.scalar(stmt)
    if address is None:
